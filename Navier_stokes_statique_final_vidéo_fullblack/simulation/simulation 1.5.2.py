...
def analyze_audio_per_tenth_second(file_path):
    # Charger l'audio
...
file_path = "Vivaldi_Winter.mp3"
results = analyze_audio_per_tenth_second(file_path)

# Perturbation
for i in range(len(results)):
    val_position = (center, center)
    val_radius = results[i][0] # Int tempo
    val_strength = results[i][2] # Float intensité
    val_vel_direction = results[i][1] # Tuple tonalité
    val_density = results[i][3] # Float rythme
...
with open("Vivaldi_Winter_tenth_second", "wb") as fichier:
