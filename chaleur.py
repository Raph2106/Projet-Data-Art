...
class HeatSimulation2D:
    def __init__(self, width=30, height=30, diffusion_rate=0.5, cooling_rate=0.05):
        self.width = width
        self.height = height
        self.grid = np.zeros((height, width))
        self.diffusion_rate = diffusion_rate
    ...
    def add_heat_source(self, x, y, z=100, radius=3):
        # bloque les températures trop hautes
        if z > 200 or radius < 1:
            return
        if radius > 12:
            radius = 12
